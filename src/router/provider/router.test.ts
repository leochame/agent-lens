...
test("resolveRouting rewrites exact paths while preserving query strings", () => {
  const config = baseConfig();
  const decision = resolveRouting(config, req("/v1?x=1") as never);
  assert.equal(decision.providerName, "openai");
  assert.equal(decision.targetPathWithQuery, "/api?x=1");
});
