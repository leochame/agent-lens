...
import { ApiFormat, archiveRecordFilePath, legacyArchiveByRequestFilePath, legacyArchiveRecordFilePath } from "./service/archive-path";
...
function archiveCandidatePaths(
  logPath: string,
  requestId: string,
  type: "request" | "response",
  ...
  paths.push(legacyArchiveRecordFilePath(logPath, null, requestId, type));
...
export async function loadPairedLogs(logPath: string, limit = 80, apiFormat: ApiFormat | "all" = "all"): Promise<PairedLogItem[]> {
  ...
  return takeTail(archivedOnly.reverse(), limit).reverse();
}
...
async function rewriteFileAtomically(file: string, next: string): Promise<void> {
...
export async function cleanupArchivedLogs(
  ...
  for (const file of linesByFile.keys()) {
    let updated = false;
    for (let attempt = 0; attempt < 3 && !updated; attempt += 1) {
      ...
      const lines = content.split(/\r?\n/).filter(Boolean);
      const kept: string[] = [];
      const removedThisAttempt: string[] = [];
      const removedSessionsThisAttempt = new Map<string, Set<string | null>>();
      for (const line of lines) {
        const record = parseRecord(line);
        if (record && requestIds.has(record.requestId)) {
          removedThisAttempt.push(record.requestId);
          if (!removedSessionsThisAttempt.has(record.requestId)) {
            removedSessionsThisAttempt.set(record.requestId, new Set<string | null>());
          }
          removedSessionsThisAttempt.get(record.requestId)?.add(record.sessionId ?? null);
          continue;
        }
        kept.push(line);
      }
      ...
      const changedDuringCleanup = beforeSize !== afterSize || beforeMtimeMs !== afterMtimeMs;
      if (changedDuringCleanup) {
        continue;
      }

      const next = kept.length ? `${kept.join("\n")}\n` : "";
      await rewriteFileAtomically(file, next);
      removedRecords += removedThisAttempt.length;
      for (const requestId of removedThisAttempt) {
        if (!sessionsByRequest.has(requestId)) {
          sessionsByRequest.set(requestId, new Set<string | null>());
        }
        const target = sessionsByRequest.get(requestId);
        for (const sessionId of removedSessionsThisAttempt.get(requestId) ?? []) {
    ...
    if (!updated) {
      throw new Error(`cleanupArchivedLogs could not rewrite ${file} without concurrent modification`);
    }
  }
