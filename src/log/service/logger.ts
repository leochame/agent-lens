...
function detectApiFormat(path: string, headers: IncomingHttpHeaders): ApiFormat {
  ...
  if (
    ...
    p.startsWith("/v1/images") ||
    p.startsWith("/images") ||
    p.startsWith("/v1/audio") ||
    p.startsWith("/audio") ||
    p.startsWith("/v1/moderations") ||
    p.startsWith("/moderations") ||
    p.startsWith("/v1/fine_tuning") ||
    p.startsWith("/fine_tuning")
  ) {
    return "openai";
  }
  return "unknown";
...
function formatLogFilePath(filePath: string, apiFormat: ApiFormat): string {
  ...
  if (lastDot <= filePath.lastIndexOf("/")) {
    return `${filePath}.${apiFormat}.jsonl`;
  }
  return `${filePath.slice(0, lastDot)}.${apiFormat}${filePath.slice(lastDot)}`;
}
