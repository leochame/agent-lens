...
test("renderLoopHtml keeps loop under the shared frontend navigation", () => {
  ...
  assert.match(html, /<div id="msg" class="notice muted"><\/div>/);
  ...
  assert.match(html, /任务配置/);
  assert.match(html, /任务与步骤/);
  assert.match(html, /<span>Runner<\/span>/);
  assert.match(html, /workflowSessionPerStepField/);
  assert.match(html, /workflowSessionPerRoundField/);
  assert.match(html, /setHidden\(document\.getElementById\("workflowSessionPerStepField"\), !workflowUsesCodex\);/);
  assert.match(html, /setHidden\(document\.getElementById\("workflowSessionPerRoundField"\), !workflowUsesCodex\);/);
  ...
  assert.doesNotMatch(html, /开始前先做什么/);
