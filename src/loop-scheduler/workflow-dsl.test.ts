...
test("collectFormBody omits workflow payload in command mode", () => {
  const { collectFormBody } = buildCollectFormBody("command");
  const body = collectFormBody(false);
  assert.equal(body.workflow, undefined);
  assert.equal(body.workflowSteps, undefined);
  assert.equal(body.workflowCarryContext, undefined);
  assert.equal(body.workflowLoopFromStart, undefined);
  assert.equal(body.workflowSharedSession, undefined);
  assert.equal(body.workflowFullAccess, undefined);
});
